...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    bs, num_heads, seq_len, d_k = key.shape # bs, seq_len, num_attention_heads, attention_head_size
    attention_scores = (query @ key.transpose(2, 3)) / math.sqrt(d_k) # Inside of softmax
    if attention_mask is not None:
      attention_scores += attention_mask
    
    # Normalize
    softmaxed = F.softmax(attention_scores, dim=-1)
    v_prime = softmaxed @ value

    # "Concatenate multi-head attention outputs to recover the original shape"
    final_attention = v_prime.transpose(1,2)
    final_attention = torch.flatten(final_attention, 2, 3)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # Add-norm
    norm_output = self.add_norm(hidden_states, att_output, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    # Feed-Forward
    interm_output = self.interm_dense(norm_output)
    interm_output = self.interm_af(interm_output)
    # Add-norm
    x = self.add_norm(norm_output, interm_output, self.out_dense, self.out_dropout, self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    x = self.embed_layer_norm(embeds)
